export type TwentyNineCmParsedItem = {
  orderNumber: string;
...
export function mapTwentyNineCmStatus(text: string): string {
  const normalized = String(text ?? "")
    .replace(/\s+/g, " ")
    .trim();

  // 29CM detail page often contains action buttons such as:
  // "주문 취소", "옵션 변경", "취소 요청".
...
export function extractTwentyNineCmSourceOrderNumberFromUrl(url: string): string | undefined {
  const patterns = [
    /my-order\/detail\/(\d+)/,
    /my-order\/orders?\/(\d+)/,
    /order-detail\/(\d+)/,
...
export function extractTwentyNineCmOrdOptNoFromUrl(url: string): string | undefined {
  const parsed = new URL(url);
  return parsed.searchParams.get("ord_opt_no") || undefined;
}

export function buildTwentyNineCmInvoiceUrl(_sourceOrderNumber: string, _ordOptNo?: string): string | undefined {
  return undefined;
}
...
export function makeTwentyNineCmOrderNumber(sourceOrderNumber: string, lineIndex: number): string {
  return `29CM-${sourceOrderNumber}-${String(lineIndex).padStart(3, "0")}`;
}
